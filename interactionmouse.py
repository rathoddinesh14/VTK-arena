...
def main(argv):
    ...
    # set the size to be 900 pixels by 900.
    #
    renWin = vtkRenderWindow()
    renWin.AddRenderer(ren1)
    renWin.SetSize(900, 900)
    renWin.SetWindowName('Interactor Style Trackball Camera')
    ...
    # current interaction style. When the user presses the 'e' / 'q' key,
    # by default an ExitEvent is invoked by the vtkRenderWindowInteractor which
    #  is caught and drops out of the event loop (triggered by the Start()
    # method that follows.
    #
    iren.Initialize()
    iren.Start()
