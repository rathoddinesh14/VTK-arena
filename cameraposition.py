...
def camera_modified_cb(caller, event):
    print(caller.GetClassName(), "modified")

    # caller is vtkOpenGLCamera
    # caller.SetDistance(5)
    # caller.SetPosition(0, 0, 5)
    # caller.SetFocalPoint(2, 2, 0)
    # caller.SetViewUp(0, 1, 0)
    # caller.SetDistance(5)

    # print clipping range
    print("Clipping range:", caller.GetClippingRange())
    caller.SetClippingRange(0.1, 100)
...
def main():
    ...
    renderer.Render()
    renderer.GetActiveCamera().AddObserver('ModifiedEvent', camera_modified_cb)
