...
def vtkCheckVersion(major, minor=0, build=0):
    '''
    Check the version of VTK installed on the system.
    @param major: Major version number.
    @param minor: Minor version number.
    @param build: Build version number.
    @return: True if the version is greater than or equal to the
    '''
    ver = vtkVersion()
    if major > ver.GetVTKMajorVersion():
        return True
    elif major == ver.GetVTKMajorVersion():
        if minor > ver.GetVTKMinorVersion():
            return True
        elif minor == ver.GetVTKMinorVersion():
            if build >= ver.GetVTKBuildVersion():
                return True
    return False
...
def main():
    ...
    if vtkCheckVersion(8, 90):
        print("Error: Requires VTK 5.10 or greater")
        return
    else:
